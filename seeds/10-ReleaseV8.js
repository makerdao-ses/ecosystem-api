...
export async function seed(knex) {
  const [techOpsMipId] = await knex("CuMip")
    .where({
      mipCode: "MIP40c3SP88",
    })
    .update({
  ...
  const [techOpsMip40Id] = await knex("Mip40")
    .insert(techOpsMip40)
    .returning("id");
  ...
  const [techOpsMipWallet40Id] = await knex("Mip40Wallet")
    .insert(techOpsMipWalletEntry)
    .returning("id");
  ...
  const [govAlphasId] = await knex("CuMip")
    .where({
      mipCode: "MIP40c3SP90",
    })
    .update({
  ...
  const [govAlphaMip40Id] = await knex("Mip40")
    .insert(govAlphaMip40)
    .returning("id");
  ...
  const [govAlphaMipWallet40Id] = await knex("Mip40Wallet")
    .insert(govAlphaMipWalletEntry)
    .returning("id");
  ...
  const [riskMip40WalletId] = await knex("Mip40Wallet")
    .insert(riskMipWalletEntry)
    .returning("id");
  ...
  const [sfMip40WalletId] = await knex("Mip40Wallet")
    .insert(sfMipWalletEntry)
    .returning("id");
  ...
  const [dinMip40WalletId] = await knex("Mip40Wallet")
    .insert(dinMipWalletEntry)
    .returning("id");
