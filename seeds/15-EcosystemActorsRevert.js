...
export async function seed(knex) {
    ...
    const groCuId = await knex('CoreUnit').where({
        shortCode: 'GRO'
    }).select('id');
    const sasCuId = await knex('CoreUnit').where({
        shortCode: 'SAS'
    }).select('id');
    const govCuId = await knex('CoreUnit').where({
        shortCode: 'GOV'
    }).select('id');
    const sesCuId = await knex('CoreUnit').where({
        shortCode: 'SES'
    }).select('id');
    const techCuId = await knex('CoreUnit').where({
        shortCode: 'TECH'
    }).select('id');

    const cuIds = [groCuId[0].id, sasCuId[0].id, govCuId[0].id, sesCuId[0].id, techCuId[0].id];

    console.log('Deleting previous ContributorTeam_AlignmentScope entries...');
    await knex('ContributorTeam_AlignmentScope')
        .whereIn('teamId', cuIds)
        .del();
    ...
    for (const scope of scopesToAdd) {
        const [{ id }] = await knex('CoreUnit').where('code', scope.shortCode).andWhere('type', 'EcosystemActor').select('id');
