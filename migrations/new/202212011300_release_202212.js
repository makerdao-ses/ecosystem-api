...
export async function up(knex) {
  ...
  await knex.schema.raw(
    `CREATE TYPE "BudgetStatus" AS ENUM ('Draft', 'Review', 'Escalated', 'Final');`,
  );
...
export async function down(knex) {
  ...
  await knex.schema.raw(`DROP TYPE "BudgetStatus" CASCADE`);
  await knex.schema.raw(
    `CREATE TYPE "BudgetStatus" AS ENUM ('Draft', 'SubmittedToAuditors', 'AwaitingCorrections', 'Final');`,
  );
  await knex.schema.alterTable("BudgetStatement", function (table) {
    table
      .enu("budgetStatus", null, {
        useNative: true,
        existingType: true,
        enumName: "BudgetStatus",
      })
