...
export async function up(knex) {
  await knex.schema
    ...
    .createTable("ChangeTrackingEvents_Index", function (table) {
      ...
      table
        .foreign("eventId")
        .references("ChangeTrackingEvents.id")
        .onDelete("CASCADE");
  ...
  await knex.raw(
    `CREATE INDEX "changeTrackingEventsIndex" ON public."ChangeTrackingEvents_Index" ("objectType" ASC, "objectId" DESC)`,
  );

  await knex.raw(
    `INSERT INTO public."ChangeTrackingEvents_Index"  (SELECT id, event_id as "eventId", 'CoreUnit' as "objectType", coreunit_id as "objectId" FROM public."ChangeTrackingEvents_CoreUnits")`,
  );
  ...
  const authorIds = await knex
    ...
    .leftJoin(
      "BudgetStatementComment_BudgetStatementCommentAuthor",
      "BudgetStatementComment.id",
      "BudgetStatementComment_BudgetStatementCommentAuthor.bsCommentId",
    )
    .leftJoin(
      "BudgetStatementCommentAuthor",
      "BudgetStatementComment_BudgetStatementCommentAuthor.bsCommentAuthorId",
      "BudgetStatementCommentAuthor.id",
    )
...
export async function down(knex) {
  ...
  console.log(
    "Dropping the status and authorId attributes from the BudgetStatementComment table... ",
  );
  await knex.schema.alterTable("BudgetStatementComment", function (table) {
