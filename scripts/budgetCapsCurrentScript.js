...
const paymentTopup = async () => {
  var result =
    await db.raw(`SELECT bsli.id as bsliid, bs.id as bsid, bsw.id as bswid, bsw.address, bs.month, bs."ownerCode", bsli."budgetCap" as "paymentTopupCap"
  ...
  paymentTopupOutput.forEach((payment) => {
    const matchedCategory = matchedCategoriesOutput.find(
      (category) =>
        ...
        payment.month.toISOString().slice(0, 10) ===
          category.month.toISOString().slice(0, 10) &&
