...
const getDistinctAddresses = async () => {
  const rows = await db('SnapshotAccount')
  ...
  await Promise.all(
    distinctAddresses.map(async (address) => {
      ...
      const [{ amount }] = await db('Snapshot')
        ...
        .leftJoin('SnapshotAccountTransaction', 'SnapshotAccount.id', 'SnapshotAccountTransaction.snapshotAccountId')
        .leftJoin('CoreUnit', 'Snapshot.ownerId', 'CoreUnit.id')
        .whereRaw('lower("SnapshotAccount"."accountAddress") = ?', [address.toLowerCase()]);

      console.log(`Address: ${address}, Balance: ${balance}, Sum of Amount: ${amount}`);
