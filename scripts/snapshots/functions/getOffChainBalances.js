...
const getBalancesFromMapping = (ownerType, ownerId) => {
  if (
    !ownerMapping[ownerType] ||
    !ownerMapping[ownerType][ownerId] ||
    !dataMapping[ownerMapping[ownerType][ownerId]]
  ) {
  ...
  const months = Object.keys(jsonData[0])
    .filter((k) => pattern.test(k))
    .map((date) => ({ key: date.slice(6, 10) + "/" + date.slice(3, 5), date }));
  const result = {};
  months.forEach((month) => {
    result[month.key] = jsonData
      .filter((entry) => !entry.Name.includes("USD value"))
      .reduce((result, entry) => {
        const { Name, Opening, ...rest } = entry;
      ...
      .filter(
        (entry) => !isNaN(entry.initialBalance) && !isNaN(entry.newBalance),
      );
  });
