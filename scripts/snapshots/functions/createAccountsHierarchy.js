const createAccountsHierarchy = async (
  snapshotReport,
  singularAccounts,
  protocolAccountId,
  ...
  accountIds.rootAccountId = await createGroupAccount(
    snapshotReport,
    "Root",
    null,
    false,
  ...
  accountIds.onchainAccountId = await createGroupAccount(
    snapshotReport,
    "On-Chain Reserves",
  ...
  accountIds.offchainAccountId = await createGroupAccount(
    snapshotReport,
    "Off-Chain Reserves",
    accountIds.coreUnitReservesAccountId,
    true,
  ...
  allAccountsInfo.upstreamAccountIdsByGroup = {
    ...allAccountsInfo.upstreamAccountIdsByGroup,
    ...newUpstreamAccountIdsByGroup,
  };
  ...
  allAccountsInfo.allAccounts = allAccountsInfo.allAccounts
    ...
    .concat(
      calculateTopLevelGroupAccountsInfo(
        allAccountsInfo,
...
const createGroupAccount = async (
  ...
  console.log(
    ` ...created new ${
      offChain ? "OFF-CHAIN " : ""
    }group account '${label}' with ID '${
      newAccount[0].id
    }'. Parent group ID is ${groupAccountId}.`,
  );
...
const initializeAllAccountsMapping = (
  ...
  singularAccounts.map((inputAccount) => {
    const result = {
      ...inputAccount,
      ...
      offChainExcluded: {
        ...
        internalAddresses: inputAccount.offChain
          ? []
          : [inputAccount.address.toLowerCase()],
        initialBalanceByToken: inputAccount.offChain
          ? {}
          : { ...inputAccount.initialBalanceByToken },
        finalBalanceByToken: inputAccount.offChain
          ? {}
          : { ...inputAccount.finalBalanceByToken },
      },
    };
...
const getAccountTypeGroup = (accountType) => {
  ...
  if (!mapping[key]) {
    throw new Error(
      `Unsupported account type for group name: "${accountType}"`,
    );
  }
...
const groupSingularAccounts = async (
  snapshotReport,
  accountIds,
  singularAccountsInfo,
  knex,
) => {
  ...
  for (let i = 0; i < singularAccountsInfo.length; i++) {
    ...
    if (!groupAccountMapping[groupName]) {
      ...
      groupAccountMapping[groupName] = account.offChain
      ...
      const childAccounts = singularAccountsInfo.filter(
        (a) => a.group == groupName,
      );
      console.log(
        ` ...creating new group account for ${childAccounts.length} '${groupName}' accounts`,
      );
      ...
      if (groupAccountMapping[groupName] === accountIds.onchainAccountId) {
        // On-chain accounts
        groupAccountMapping[groupName] = await createGroupAccount(
          snapshotReport,
          groupName,
          accountIds.onchainAccountId,
          false,
  ...
  await setSingularAccountsGroupIds(
    singularAccountsInfo,
    groupAccountMapping,
    knex,
  );
...
const setSingularAccountsGroupIds = async (
  singularAccountsInfo,
  groupAccountMapping,
  knex,
) => {
  console.log(
    ` ...updating groupAccountId of ${singularAccountsInfo.length} singular accounts`,
  );
  ...
  for (let i = 0; i < singularAccountsInfo.length; i++) {
    ...
    if (!groupName) {
      console.error(
        ` ...ERROR: expected group to be set on singular account`,
        singularAccountsInfo[i],
      );
    } else if (!groupAccountMapping[groupName]) {
      console.error(
        ` ...ERROR: expected groupAccountId for group '${groupName}' set in `,
        groupAccountMapping,
      );
    } else {
...
const calculateTopLevelGroupAccountsInfo = (
  singularAccountsInfo,
  otherGroupAccounts,
  accountIds,
) => {
  ...
  newGroupAccountsInfo.push({
    ...
    address: "0xGROUP",
    ...calculateParentFromDirectChildren(
      singularAccountsInfo.allAccounts
        .concat(otherGroupAccounts)
        .filter((a) => onChainReserveChildrenIds.indexOf(a.accountId) > -1),
      accountIds.onchainAccountId,
    ),
  });
  ...
  newGroupAccountsInfo.push({
    ...
    address: "0xGROUP",
    ...calculateParentFromDirectChildren(
      singularAccountsInfo.allAccounts.filter(
        (a) => a.group == "PaymentProcessor",
      ),
      accountIds.offchainAccountId,
    ),
  });
  ...
  newGroupAccountsInfo.push({
    ...
    address: "0xGROUP",
    ...calculateParentFromDirectChildren(
      newGroupAccountsInfo.filter(
        (a) =>
          [accountIds.offchainAccountId, accountIds.onchainAccountId].indexOf(
            a.accountId,
          ) > -1,
      ),
      accountIds.coreUnitReservesAccountId,
      ...
      singularAccountsInfo.allAccounts
        ...
        .filter(
          (a) =>
            [
              accountIds.coreUnitReservesAccountId,
              accountIds.protocolAccountId,
            ].indexOf(a.accountId) > -1,
        ),
      accountIds.rootAccountId,
    ),
  });
...
const getGroupUpstream = (groupName, allAccounts) => {
  const mapping = {
    PaymentProcessor: ["Protocol", "DSSVest", "Auditor", "Operational"],
    Operational: ["Protocol", "DSSVest", "Auditor"],
    Auditor: ["Protocol", "DSSVest"],
    DSSVest: ["Protocol"],
    Reserve: ["Protocol"],
    Protocol: [null],
    Root: [null],
...
const setUpstreamAccountIds = async (allAccountsInfo, knex) => {
  ...
  console.log(
    ` ...updating upstreamAccountId of ${allAccountsInfo.allAccounts.length} accounts`,
  );
  for (let i = 0; i < allAccountsInfo.allAccounts.length; i++) {
    const upstreamGroupName = getGroupUpstream(
      allAccountsInfo.allAccounts[i].group,
      allAccountsInfo.allAccounts,
    );

    if (upstreamGroupName !== null) {
      const upstreamGroupId =
        allAccountsInfo.upstreamAccountIdsByGroup[upstreamGroupName];
      ...
      downstreamSet[upstreamGroupId].push(
        allAccountsInfo.allAccounts[i].accountId,
      );
    }
  }
...
const mergeAddObjects = (a, b) => {
  ...
  Object.keys(b).forEach(
    (k) => (result[k] = result[k] ? result[k] + b[k] : b[k]),
  );
  return result;
};
...
const calculateParentFromDirectChildren = (childAccounts, parentId) => {
  const result = {
    ...
    offChain: childAccounts
      .map((c) => c.offChain)
      .reduce((prev, next) => prev && next, true),

    addedTransactions: childAccounts
      .map((c) => c.addedTransactions)
      .reduce((prev, next) => prev + next, 0),

    timespan: {
      start: childAccounts
        .map((c) => c.timespan.start)
        .reduce(
          (prev, next) => ((prev && prev < next) || !next ? prev : next),
          null,
        ),
      end: childAccounts
        .map((c) => c.timespan.end)
        .reduce(
          (prev, next) => ((prev && prev > next) || !next ? prev : next),
          null,
        ),
    ...
    offChainIncluded: {
      initialBalanceByToken: childAccounts
        .map((c) => c.offChainIncluded.initialBalanceByToken)
        .reduce((prev, next) => mergeAddObjects(prev, next), {}),
      finalBalanceByToken: childAccounts
        .map((c) => c.offChainIncluded.finalBalanceByToken)
        .reduce((prev, next) => mergeAddObjects(prev, next), {}),
      internalIds: childAccounts
        .map((c) => c.offChainIncluded.internalIds)
        .reduce((prev, next) => prev.concat(next), [parentId]),
      internalAddresses: childAccounts
        .map((c) => c.offChainIncluded.internalAddresses)
        .reduce((prev, next) => prev.concat(next), []),
    },

    offChainExcluded: {
      initialBalanceByToken: childAccounts
        .map((c) => c.offChainExcluded.initialBalanceByToken)
        .reduce((prev, next) => mergeAddObjects(prev, next), {}),
      finalBalanceByToken: childAccounts
        .map((c) => c.offChainExcluded.finalBalanceByToken)
        .reduce((prev, next) => mergeAddObjects(prev, next), {}),
      internalIds: childAccounts
        .map((c) => c.offChainExcluded.internalIds)
        .reduce((prev, next) => prev.concat(next), [parentId]),
      internalAddresses: childAccounts
        .map((c) => c.offChainExcluded.internalAddresses)
        .reduce((prev, next) => prev.concat(next), []),
    },
  };
