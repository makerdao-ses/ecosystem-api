const insertMissingPaymentProcessorTransactions = async (processorAccountId, monthInfo, knex) => {

    let newBalance = 0;
    if (monthInfo.month && monthInfo.offChainBalances[monthInfo.month]) {
        const usdRecord = monthInfo.offChainBalances[monthInfo.month].filter(b => b.token === 'USD');
        ...
        if (months.length > 0) {
            const usdRecord = monthInfo.offChainBalances[months[months.length - 1]].filter(b => b.token === 'USD');
            if (usdRecord.length>0) {
