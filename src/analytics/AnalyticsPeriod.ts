import { AnalyticsSerializerTypes } from "./AnalyticsSerializer.js";
...
export class AnalyticsPeriod {
    get type(): AnalyticsPeriodType { return this._type; }
    ...
    public static fillRange(p1:AnalyticsPeriod | string, p2:AnalyticsPeriod | string): AnalyticsPeriod[] {
    ...
    private _addPeriods(periods: number): AnalyticsPeriod {
        ...
        if (this._type === AnalyticsPeriodType.Year) {
            ...
            const [y, q] = AnalyticsPeriod.normalizeQuarters(this._year, (this._quarter as number) + periods);
            result = new AnalyticsPeriod(y, q);

        } else {
            const [y, _, m] = AnalyticsPeriod.normalizeMonths(this._year, (this._month as number) + periods);
