...
export class MipModel {
  ...
  async getMips(filter?: CuMipFilter): Promise<CuMip[]> {
    const baseQuery = this.knex.select("*").from("CuMip").orderBy("id");
  ...
  async getMipReplaces(filter?: MipReplaceFilter): Promise<MipReplaces[]> {
    const baseQuery = this.knex.select("*").from("MipReplaces").orderBy("id");
    if (filter?.id !== undefined) {
      return baseQuery.where("id", filter.id);
  ...
  async getMip39s(filter?: Mip39Filter): Promise<Mip39[]> {
    const baseQuery = this.knex.select("*").from("Mip39").orderBy("id");
    if (filter?.id !== undefined) {
      return baseQuery.where("id", filter.id);
  ...
  async getMip40s(filter?: Mip40Filter): Promise<Mip40[]> {
    const baseQuery = this.knex.select("*").from("Mip40").orderBy("id");
    if (filter?.id !== undefined) {
      return baseQuery.where("id", filter.id);
  ...
  async getMip40BudgetPeriods(
    filter: Mip40BudgetPeriodFilter,
  ): Promise<Mip40BudgetPeriod[]> {
  ...
  async getMip40Wallets(filter?: Mip40WalletFilter): Promise<Mip40Wallet[]> {
    const baseQuery = this.knex.select("*").from("Mip40Wallet").orderBy("id");
    if (filter?.id !== undefined) {
      return baseQuery.where("id", filter.id);
    } else if (filter?.mip40Id !== undefined) {
      return baseQuery.where("mip40Id", filter.mip40Id);
  ...
  async getMip40BudgetLineItems(
    filter?: Mip40BudgetLineItemFilter,
  ): Promise<Mip40BudgetLineItem[]> {
    const baseQuery = this.knex
      .select("*")
    ...
    if (filter?.id !== undefined) {
      ...
      return baseQuery.where(
        ...
        filter.canonicalBudgetCategory,
      );
  ...
  async getMip41s(filter?: Mip41Filter): Promise<Mip41[]> {
    const baseQuery = this.knex.select("*").from("Mip41").orderBy("id");
    if (filter?.id !== undefined) {
      return baseQuery.where("id", filter.id);
    } else if (filter?.cuMipId !== undefined) {
      return baseQuery.where("cuMipId", filter.cuMipId);
...
export default (knex: Knex, deps: { [key: string]: object }) =>
  new MipModel(knex, deps["CoreUnit"]);
