...
it('returns list of Mip40BudgetPeriods with or no params', async () => {
    ...
    const entry3 = await authModel.getMip40BudgetPeriods({ budgetPeriodStart: '2022-01-01' });
    const entry4 = await authModel.getMip40BudgetPeriods({ budgetPeriodEnd: '2022-01-01' });
...
it('returns list of Mip40BudgetLineItem with mip40WalletId or undefined as params', async () => {
    ...
    const entry4 = await authModel.getMip40BudgetLineItems({ budgetCategory: '' });
    const entry5 = await authModel.getMip40BudgetLineItems({ budgetCap: 1 });
    const entry6 = await authModel.getMip40BudgetLineItems({ canonicalBudgetCategory: 'GasExpense' });
    const entry7 = await authModel.getMip40BudgetLineItems({ group: '' });
    const entry8 = await authModel.getMip40BudgetLineItems({ headcountExpense: true });
    expect(entry.length).toBeGreaterThan(0);
    expect(entry1.length).toBeGreaterThan(0);
});
