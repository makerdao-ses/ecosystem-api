...
it("get systemRoleMembers", async () => {
  const entry = await authModel.getSystemRoleMembers(
    "CoreUnitFacilitator",
    "CoreUnit",
    45,
  );
  const entry1 = await authModel.getSystemRoleMembers(
    "CoreUnitAuditor",
    "CoreUnit",
    null,
  );
...
it("gets usersFiltered", async () => {
  const entry = await authModel.getUsersFiltered({
    id: 1,
    active: true,
    rolesAndPermissions: false,
  });
  const entry1 = await authModel.getUsersFiltered({
    id: 2,
    active: true,
    rolesAndPermissions: true,
  });
  const entry2 = await authModel.getUsersFiltered({
    username: "exampleName",
    active: true,
    rolesAndPermissions: true,
  });
  const entry3 = await authModel.getUsersFiltered({
    rolesAndPermissions: true,
    ids: [1, 2],
  });
