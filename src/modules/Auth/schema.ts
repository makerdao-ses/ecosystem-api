...
export const resolvers = {
  ...
  Mutation: {
    userLogin: async (_: any, { input }: any, { dataSources }: any) => {
      try {
        const [user] = await dataSources.db.Auth.getUser(
          "username",
          input.username,
        );
        if (user != undefined) {
          ...
          if (match === true) {
            ...
            if (result.length > 0) {
              ...
              userObj = await dataSources.db.Auth.getUser(
                "username",
                input.username,
              );
            }
    ...
    userCreate: async (
      ...
      try {
        if (!user && !auth) {
          ...
          const allowed = await dataSources.db.Auth.canManage(
            user.id,
            "System",
          );
          if (allowed[0].count > 0) {
            ...
            if (user === undefined && regexPw(input.password)) {
              ...
              const userCreate = await dataSources.db.Auth.createUser(
                input.username,
                hash,
              );
              return userCreate;
        ...
        if (user) {
          ...
          if (allowed[0].count > 0 && regexPw(input.newPassword)) {
            ...
            if (match) {
              ...
              await dataSources.db.Auth.changeUserPassword(
                input.username,
                hash,
              );
      ...
      try {
        ...
        if (!user && !auth) {
          ...
          if (allowed[0].count > 0) {
            ...
            if (result.length < 1) {
              const [userFrobDB] = await dataSources.db.Auth.getUser(
                "id",
                user.id,
              );
...
const buildRoleObjectFromGroupedRows = (rows: any) => {
  const result = {
    ...
    permissions: rows
      .filter((row: any) => row.resource !== null)
      .map((row: any) => {
...
const regexPw = (pw: string) => {
  ...
  const matchesLowerCaseChars = (pw: string) => {
    ...
    if (count >= 1) {
      ...
      throw Error(
        "Your password must contain at least one lowercase character, number, or special character.",
      );
    }
  };
  ...
  const matchesUpperCaseChars = (pw: string) => {
    ...
    if (count >= 1) {
      ...
      throw Error(
        "Your password must contain at least one uppercase character, number, or special character.",
      );
    }
  };
  ...
  const matchesNumbers = (pw: string) => {
    ...
    if (count > 0) {
      ...
      throw Error(
        "Your password must contain at least one lowercase character, uppercase character, or special character.",
      );
    } else {
      throw Error(
        "Your password must contain at least one uppercase character, lowercase character, number, or special character.",
      );
    }
  };
  ...
  if (matchClasses < 2) {
    throw Error(
      "Password needs to match at least 2 of: lowercase chars, uppercase chars, numbers, special chars.",
    );
  } else {
    return true;
  }
