...
export class AuthModel {
  ...
  async canUpdate(
    ...
    return this.knex
      ...
      .leftJoin("RolePermission", function () {
        this.on("UserRole.roleId", "=", "RolePermission.roleId").andOn(
          "UserRole.resource",
          "=",
  ...
  async createUser(username: string, password: string): Promise<any> {
    const user = await this.knex("User")
      .insert({ username, password, active: true })
      .returning("*");
    return {
      id: user[0].id,
  ...
  async getSystemRoleMembers(
    ...
    const baseQuery = this.knex
      ...
      .leftJoin("UserRole", function () {
        this.on("Role.id", "=", "UserRole.roleId");
      })
      .where("system", true)
  ...
  async getUsers(
    ...
    const baseQuery = this.knex
      ...
      .leftJoin("RolePermission", function () {
        this.on("RolePermission.roleId", "=", "UserRole.roleId").andOn(
          "UserRole.resource",
          "=",
          "RolePermission.resource",
        );
