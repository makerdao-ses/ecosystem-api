...
export class BudgetModel {
  ...
  processBudgets(
    ...
    for (const budget of budgets) {
      if (
        ...
        (idOrCode.code && budget.code == idOrCode.code)
      ) {
        result.push(budget);
  ...
  async getBudgetCaps(budgetId: number | string) {
    return this.knex.select("*").from("BudgetCap").where("budgetId", budgetId);
  }
  ...
  async updateBudget(updatedFields: Partial<Budget>) {
    ...
    if (parentId) {
      ...
      if (idPath.includes(parentId)) {
        throw new Error(
          `Can't change parentId to ${parentId} because it would create a circular dependency`,
        );
      }
    }
    delete updatedFields.id;
    const [result] = await this.knex("Budget")
      .where("id", id)
  ...
  async deleteBudget(id: number | string) {
    ...
    const [childBudgetCount] = await this.knex("Budget")
      .where("parentId", id)
      .count("parentId");
    // check if any budget caps exist with this budget
    const [budgetCapCount] = await this.knex("BudgetCap")
      .where("budgetId", id)
      .count("budgetId");
    if (
      Number(budgetCapCount.count) > 0 ||
      Number(childBudgetCount.count) > 0
    ) {
      throw new Error(
        `Cannot delete budget ${name} with ID ${id} because it has budget caps or child budgets`,
      );
    } else {
      return this.knex("Budget").where("id", id).del();
    }
  }
  ...
  async deleteBudgetCap(id: number | string) {
    return this.knex("BudgetCap").where("id", id).returning("*").del();
  }

  // delete expense category (if no line items and caps exist)
  async deleteExpenseCategory(id: number | string) {
    const [{ name }] = await this.knex("ExpenseCategory")
      .where("id", id)
      .select("name");

    // check if any line items exist wtih this expense category
    // check if budget caps exist with this expense category
    const [lineItemCount] = await this.knex("BudgetStatementLineItem")
      .where("canonicalBudgetCategory", name)
      .count("canonicalBudgetCategory");
    const [budgetCapLineItemCount] = await this.knex("Mip40BudgetLineItem")
      .where("canonicalBudgetCategory", name)
      .count("canonicalBudgetCategory");
    console.log(lineItemCount.count, budgetCapLineItemCount.count);

    if (
      Number(lineItemCount.count) > 0 &&
      Number(budgetCapLineItemCount.count) > 0
    ) {
      throw new Error(
        `Cannot delete expense category ${name} with ID ${id} because it has line items`,
      );
    } else {
      return this.knex("ExpenseCategory").where("id", id).del();
    }
  }

  async getExpenseCategories() {
    return this.knex.select("*").from("ExpenseCategory").orderBy("id", "asc");
  }
}
