...
export class LineItemFetcher {
  ...
  public static lineItemGroupToString(lineItems: LineItemGroup): string {
    ...
    result += ` - latestReport:${lineItems.latestReport?.toString()} - hasActuals:${
      lineItems.hasActuals
    }`;
    result += ` - obsolete:${lineItems.categories.reduce(
      (obs, cat) => obs + (cat.obsolete ? 1 : 0),
      0,
    )}`;
    result += ` - fteCap:${
      lineItems.fteCap === null ? "(not set)" : lineItems.fteCap
    }\n\n`;
    ...
    result +=
      "   " +
      LineItemFetcher._padString("GROUP", 15) +
      " " +
  ...
  private static _padNumber(n: number, length: number) {
    const numberString = Intl.NumberFormat("en-US", {
  ...
  public async getCaps(
    ...
    const query = this._knex
      ...
      .innerJoin(
        "public.Mip40BudgetLineItem AS MBLI",
        "MW.id",
        "MBLI.mip40WalletId",
      )
  ...
  public async getAvailableMonthsRange() {
    const result = await this._knex("public.BudgetStatementLineItem as BSLI")
  ...
  public buildQuery(owner: string, account: string, month: string) {
    const q = this._knex
      ...
      .leftJoin(
        "public.BudgetStatementLineItem as BSLI",
        "BSLI.budgetStatementWalletId",
        "BSW.id",
      )

      .whereRaw(
        '(("BS"."ownerType" NOT IN (\'CoreUnit\', \'EcosystemActor\')) OR ("BS"."ownerCode" = ?))',
        owner,
      )
  ...
  public async getLineItems(
    ...
    const capNumbers = await this.getCaps(
      owner,
      account,
      result.month.startAsSqlDate(),
    );
    if (capNumbers.length > 0) {
      ...
      const budgetCapTotal = result.categories.reduce(
        (total, category) => total + (category.numbers.budgetCap || 0.0),
        0.0,
      );
      const remainderCap = capNumbers[0].budgetCap - budgetCapTotal;

      if (remainderCap > 0.0 || remainderCap < 0.0) {
  ...
  private async _readLineItemRecords(
    owner: string,
    account: string,
    month: string,
  ): Promise<LineItemGroup> {
    ...
    records.forEach((r: any) => {
      ...
      if (
        ...
        currentCategory = this._newCategory(
          group,
          r.headcountExpense,
          r.category,
        );
      ...
      if (report.comesBefore(result.month)) {
        ...
        currentCategory.numbers = {
          actual: 0.0,
          forecast: Number.parseFloat(r.forecast) || 0.0,
          budgetCap: Number.parseFloat(r.budgetCap) || 0.0,
          payment: 0.0,
        };
        ...
        currentCategory.numbers = {
          actual: Number.parseFloat(r.actual) || 0.0,
          forecast: Number.parseFloat(r.forecast) || 0.0,
          budgetCap: Number.parseFloat(r.budgetCap) || 0.0,
          payment: Number.parseFloat(r.payment) || 0.0,
        };
      }
      ...
      if (
        result.latestReport === null ||
        result.latestReport.comesBefore(report)
      ) {
        result.latestReport = report;
      }

      result.hasActuals =
        result.hasActuals || currentCategory.numbers.actual > 0.0;
    });
  ...
  private _hasActualsOrPayments(record: any): boolean {
    return (
      (record.actual && Number.parseFloat(record.actual) > 0.0) ||
      (record.payment && Number.parseFloat(record.payment) > 0.0)
    );
  }
  ...
  private _parseDateStringAsMonthPeriod(date: string): BudgetReportPeriod {
    return BudgetReportPeriod.fromString(
