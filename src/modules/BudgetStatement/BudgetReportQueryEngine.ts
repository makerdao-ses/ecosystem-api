...
export class BudgetReportQueryEngine {
  get rootResolver() {
    return this._resolvers[this._rootResolver] as BudgetReportResolver<
      ResolverData,
      any
    >;
  }
  ...
  ): Promise<BudgetReportOutputGroup[]> {
    const budgetPath =
      typeof query.budgets == "string"
        ? BudgetReportPath.fromString(query.budgets)
        : query.budgets;
    const categoryPath =
      typeof query.categories == "string"
        ? BudgetReportPath.fromString(query.categories)
        : query.categories;
  ...
  private async _callResolvers(
    rootResolverQuery: ResolverData,
  ): Promise<BudgetReportOutputGroup[]> {
    ...
    const outputStack: OutputStackElement[] = [
      { preprocessed: false, output: [] },
    ];
    ...
    while (resolverStack.length > 0) {
      ...
      const resolver = this._resolvers[
        nextResolverElement.name
      ] as BudgetReportResolver<ResolverData, ResolverData>;
      if (!resolver) {
        throw new Error(
          `Resolver '${nextResolverElement.name}' is selected as next resolver, but it cannot be found.`,
        );
      }
      ...
      if (nextResolverElement.state === ResolverStackState.ReadyForQuery) {
        ...
        nextResolverElement.state === ResolverStackState.ReadyForProcessing
      ) {
        // Grab the collected output of the subsequent resolvers for processing
        const batchResults = outputStack.pop();
        if (!batchResults) {
          throw new Error(
            `Unexpected end of BudgetReportQueryEngine collected output stack.`,
          );
        }
        ...
        if (batchResults.preprocessed) {
          ...
          if (DEBUG_OUTPUT) {
            console.log(
              ` Skipping output group processing of ${resolver.name} since the output was preprocessed.`,
            );
          }
        } else {
          ...
          if (this._resolverCache && resolver.supportsCaching()) {
            if (DEBUG_OUTPUT) {
              console.log(
                "BudgetReportQuery is storing batch result with ",
                processedOutput.length,
                " groups for ",
                JSON.stringify(
                  this._getBatchCacheKeys(
                    resolver,
                    nextResolverElement.queries,
                  ),
                ).slice(0, 300),
            ...
            await this._resolverCache.store(
              this._getBatchCacheKeys(resolver, nextResolverElement.queries),
              processedOutput,
            );
          }
        }
        ...
        outputStack[outputStack.length - 1].output =
          outputStack[outputStack.length - 1].output.concat(processedOutput);
      }
            ...
            console.log(
              " Cache stats: \n  " +
                Object.keys(stats)
                  .map(
                    (res) =>
                      `${res} [${stats[res].hits} hits | ${stats[res].misses} misses | ${stats[res].stores} stores]`,
                  )
                  .join("\n  ") +
                "\n",
            );
  ...
  private _getBatchCacheKeys(
    resolver: BudgetReportResolver<ResolverData, ResolverData>,
  ...
  private async _executeBatch(
    ...
    if (this._resolverCache && resolver.supportsCaching()) {
      cachedOutput = await this._resolverCache.load(
        this._getBatchCacheKeys(resolver, queries),
      );
    }
    ...
    if (cachedOutput) {
      ...
      for (const query of queries) {
        const resolverOutput: ResolverOutput<ResolverData> =
          await this._executeQuery(resolver, query);
        ...
        Object.keys(resolverOutput.nextResolversData).forEach((k) => {
          ...
          result.nextResolversData[k] = result.nextResolversData[k].concat(
            resolverOutput.nextResolversData[k],
          );
        });
    ...
    if (DEBUG_OUTPUT) {
      if (result.output.length > 0) {
        console.log(
          ...
          "resulting groups from",
          queries.length,
          resolver.name,
          "execute calls, for example:\n",
          JSON.stringify(
            result.output[Math.floor(result.output.length / 2)],
          ).slice(0, 300) + " ...",
        );
      } else {
        console.log(
          "  No results collected from",
          queries.length,
          resolver.name,
          "execute calls.",
        );
  ...
  private async _executeQuery(
    ...
    if (!cachedOutput) {
      ...
      const resultsAreComplete =
        Object.keys(result.nextResolversData).length < 1;
  ...
  private _visualizeResolverStack(
    resolverStack: ResolverStackElement[],
    collectedOutputStack: OutputStackElement[],
  ): string {
    let result = "[ RESULT ] << ",
      waitingResolvers: string[] = [];
    ...
    if (resolverStack.length > 0) {
      if (
        resolverStack[resolverStack.length - 1].state ===
        ResolverStackState.ReadyForQuery
      ) {
        result +=
          "QRY\n" +
          collectedOutputStack
            .map(
              (outputElement) => " " + outputElement.output.length + " groups ",
            )
            .join(" || ") +
          "\n";
      } else {
        result +=
          ...
          collectedOutputStack
            ...
            .join(" || ") +
          " <- " +
          collectedOutputStack.slice(-1)[0].output.length +
          " groups\n";
      }
    } else {
