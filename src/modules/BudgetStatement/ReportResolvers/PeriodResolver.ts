...
} from "../BudgetReportPeriod.js";
import {
  BudgetReportGranularity,
  BudgetReportPeriodInput,
} from "../BudgetReportQuery.js";
import {
...
export class PeriodResolver extends BudgetReportResolverBase<
  ResolverData,
  PeriodResolverData
> {
  ...
  public async execute(
    query: ResolverData,
  ): Promise<ResolverOutput<PeriodResolverData>> {
    if (DEBUG_OUTPUT) {
      console.log(
        `PeriodResolver is resolving budgets "${query.budgetPath.toString()}" with categories "${query.categoryPath.toString()}"`,
      );
    }
    ...
    if (periodRange[0].type !== BudgetReportPeriodType.Month) {
      throw new Error(
        "Quarters and years are not allowed as query start or end values. Use the respective months instead.",
      );
    }
  ...
  public processOutputGroups(
    groups: BudgetReportOutputGroup[],
  ): BudgetReportOutputGroup[] {
    ...
    for (const group of groups) {
      group.rows.forEach((r) => {
        ...
        if (typeof result[groupName] == "undefined") {
          ...
          result[groupName].rows[0].forecastDiscontinued +=
            r.forecastDiscontinued;
          result[groupName].rows[0].budgetCapDiscontinued +=
            r.budgetCapDiscontinued;
          result[groupName].rows[0].paymentDiscontinued +=
            r.paymentDiscontinued;
          result[groupName].rows[0].predictionDiscontinued +=
            r.predictionDiscontinued;
        }
      });
    }
  ...
  private _groupByGranularity(
    range: BudgetReportPeriod[],
    granularity: BudgetReportGranularity,
  ): Record<string, BudgetReportPeriod[]> {
  ...
  private async _resolvePeriodRange(
    start: BudgetReportPeriodInput,
    end: BudgetReportPeriodInput,
  ): Promise<BudgetReportPeriod[]> {
    let first: BudgetReportPeriod, last: BudgetReportPeriod;

    if (start === null || end === null) {
      const availableMonths =
        await this._lineItemFetcher.getAvailableMonthsRange();

      first = availableMonths.first;
      if (start !== null) {
        first =
          typeof start === "string"
            ? BudgetReportPeriod.fromString(start)
            : start;
      }
      ...
      if (end !== null) {
        last =
          typeof end === "string" ? BudgetReportPeriod.fromString(end) : end;
      }
    } else {
      first =
        typeof start === "string"
  ...
  private _getGroupName(
    ...
    if (this._granularity === BudgetReportGranularity.Annual) {
      ...
      result +=
        ...
        row.month.year +
        "/" +
        ((row.month.month as number) < 10 ? "0" : "") +
        row.month.month;
    }
