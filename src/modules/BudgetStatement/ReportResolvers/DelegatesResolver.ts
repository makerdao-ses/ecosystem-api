...
export class DelegatesResolver extends BudgetReportResolverBase<
  PeriodResolverData,
...
> {
  ...
  public async execute(
    ...
    if (DEBUG_OUTPUT) {
      console.log(
        `DelegatesResolver is resolving ${query.budgetPath.toString()}`,
      );
    }
    ...
    const resolverInput: AccountsResolverData[] = delegatesWallets.map(
      (wallet) => {
        const result = {
          owner: "Delegates",
