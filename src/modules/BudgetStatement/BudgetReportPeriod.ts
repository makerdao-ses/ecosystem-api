...
export class BudgetReportPeriod {
  get type(): BudgetReportPeriodType {
    return this._type;
  }
  get year(): number {
    return this._year;
  ...
  public static fillRange(
    p1: BudgetReportPeriod | string,
    p2: BudgetReportPeriod | string,
  ): BudgetReportPeriod[] {
    const obj1 =
        typeof p1 === "string" ? BudgetReportPeriod.fromString(p1) : p1,
  ...
  public toString(): string {
    let result =
      this._year + (this._type === BudgetReportPeriodType.Year ? "" : "/");
  ...
  private _addPeriods(periods: number): BudgetReportPeriod {
    ...
    if (this._type === BudgetReportPeriodType.Year) {
      ...
      const [y, q] = BudgetReportPeriod.normalizeQuarters(
        this._year,
        (this._quarter as number) + periods,
      );
      result = new BudgetReportPeriod(y, q);
    } else {
      const [y, _, m] = BudgetReportPeriod.normalizeMonths(
        this._year,
