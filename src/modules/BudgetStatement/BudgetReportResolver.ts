...
export interface BudgetReportResolver<
  TInput extends ResolverData,
  TOutput extends ResolverData,
> extends NamedResolver {
  execute(query: TInput): Promise<ResolverOutput<TOutput>>;
  processOutputRows(
    rows: BudgetReportOutputRow[],
    groupKeys: Record<string, SerializableKey>,
...
export abstract class BudgetReportResolverBase<
  TInput extends ResolverData,
  TOutput extends ResolverData,
> implements BudgetReportResolver<TInput, TOutput>
{
  ...
  public processOutputRows(
    rows: BudgetReportOutputRow[],
    groupKeys: Record<string, any>,
  ): BudgetReportOutputRow[] {
    if (DEBUG_OUTPUT) {
