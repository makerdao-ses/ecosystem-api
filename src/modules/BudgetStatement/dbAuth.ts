...
export class BudgetStatementAuthModel {
  ...
  constructor(
    bsModel: BudgetStatementModel,
    authModel: AuthModel,
    cuModel: CoreUnitModel,
    ctModel: ChangeTrackingModel,
  ) {
  ...
  async budgetStatementCommentCreate(input: any, user: any) {
    try {
      if (!user) {
        ...
        const [ownerTypeResult] = await this.bsModel.getBSOwnerType(
        ...
        if (
          input.comment === null ||
          (input.comment === "" && budgetStatement.status === input.status)
        ) {
          throw new ForbiddenError("Need to add a comment or change status");
        }
        const canUpdate = await this.authModel.canUpdateCoreUnit(
          user.id,
          ownerTypeResult.ownerType,
          budgetStatement.ownerId,
        );
        const canAudit = await this.authModel.canAudit(
          user.id,
          ownerTypeResult.ownerType,
        );
        ...
        const userHasAuditPermission = () => {
          ...
          canAudit.forEach((obj: any) => {
            if (
              obj.resourceId === budgetStatement.ownerId ||
              obj.resourceId === null
            ) {
              auditor = true;
            }
        ...
        if (oldStatus != newStatus) {
          ...
          if (!isAllowed[oldStatus][newStatus]) {
            throw new ForbiddenError(
              `You are not authorized to move the expense report status from ${oldStatus} to ${newStatus}.`,
            );
          }

          const addedComment = await this.bsModel.addBudgetStatementComment(
          ...
          await createBudgetStatementCommentEvent(
            this.bsModel,
            this.cuModel,
            this.ctModel,
            parsed[0],
            budgetStatement.status,
            budgetStatement.ownerType,
          );
          return parsed;
        }
        ...
        if (input.comment != null || input.comment != undefined) {
          const canComment =
            userHasUpdatePermission() || userHasAuditPermission();

          if (!canComment) {
            throw new ForbiddenError(
              "You are not authorized to add a comment to this expense report.",
            );
          }
...
const createBudgetStatementCommentEvent = async (
  bsModel: BudgetStatementModel,
  cuModel: CoreUnitModel,
  ctModel: ChangeTrackingModel,
  parsedComment: any,
  ...
  if (ownerType === "Delegates") {
    ...
    [CU] = await cuModel.getCoreUnits({
      filter: { id: budgetStatement.ownerId },
    });
  }
  const eventDescription = getEventDescription(
    ...
    budgetStatement.month.substring(0, budgetStatement.month.length - 3),
    ownerType,
  );
