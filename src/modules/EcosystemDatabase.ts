...
export default class EcosystemDatabase extends SQLDataSource {
  ...
  loadModule(
    moduleName: string,
    apiModule: DatabaseFactoryFn,
    dependencies: string[],
  ): void {
    if (typeof apiModule !== "function") {
      throw new Error(
        `The apiModule parameter for module ${moduleName} must be a function.`,
      );
    }

    if (DEBUG_OUTPUT) {
      console.log(
        `Loading database object for API module '${moduleName}' with dependencies [` +
          dependencies.join(", ") +
          "]",
      );
    }
    ...
    dependencies.forEach((d: string) => {
      if (!this[d]) {
        throw Error(
          `Dependency ${d} not found while loading module '${moduleName}'`,
        );
      }
