...
export class CoreUnitModel {
  ...
  getCuUpdates(filter?: {
    ...
    const baseQuery = this.knex.select("*").from("CuUpdate").orderBy("id");
  ...
  getContributors(filter: {
    ...
    const baseQuery = this.knex.select("*").from("Contributor").orderBy("id");
    if (filter.limit !== undefined && filter.offset !== undefined) {
