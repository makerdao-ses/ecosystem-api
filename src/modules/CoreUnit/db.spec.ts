...
it("returns contributor commitment with or no params", async () => {
  ...
  const entry2 = await authModel.getContributorCommitments({
    contributorId: 1,
  });
  const entry3 = await authModel.getContributorCommitments({
    startDate: "2022-01-01",
  });
  const entry4 = await authModel.getContributorCommitments({
    commitment: "FullTime",
  });
  const entry5 = await authModel.getContributorCommitments({
    cuCode: "SES-001",
  });
  const entry6 = await authModel.getContributorCommitments({
    jobTitle: "supremeLord",
  });
...
it("returns list of contributors with and no params", async () => {
  ...
  const entry3 = await authModel.getContributors({
    filter: { name: "Payton" },
  });
  const entry5 = await authModel.getContributors({
    filter: { forumHandle: "@" },
  });
  const entry6 = await authModel.getContributors({
    filter: { discordHandle: "@" },
  });
  const entry7 = await authModel.getContributors({
    filter: { twitterHandle: "@" },
  });
  const entry8 = await authModel.getContributors({
    filter: { email: "a@a.as" },
  });
  expect(entry.length).toBeGreaterThan(0);
  expect(entry1.length).toBeGreaterThan(0);
...
it("returns list of githubEcosystem with or no params", async () => {
  ...
  const entry4 = await authModel.getMakerGithubEcosystemAll({
    date: "2022-01-01",
  });
  ...
  const entry8 = await authModel.getMakerGithubEcosystemAll({
